...
func main() {
	...
	cacheClient = NewMemcache("tcp", fmt.Sprintf("%s:%s", memcachedHost, memcachedPort))
...
func getUserSimpleByID(q sqlx.Queryer, userID int64) (userSimple UserSimple, err error) {
	user, err := GetUserCacheByID(userID)
	if err != nil {
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	tsDict, err := FetchTransactionDictByItems(items)
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		return
...
func postItemEdit(w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now()
	_, err = dbx.Exec("UPDATE `items` SET `price` = ?, `updated_at` = ? WHERE `id` = ? AND status = ?",
		price,
		now,
		itemID,
		ItemStatusOnSale,
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	...
	ssrRes := make(chan *APIShipmentStatusRes)
...
func postSell(w http.ResponseWriter, r *http.Request) {
	...
	seller, err := GetUserCacheByID(user.ID)
	if seller == nil {
		outputErrorMsg(w, http.StatusNotFound, "user not found")
		return
	}
...
func postLogin(w http.ResponseWriter, r *http.Request) {
	...
	u, err := GetUserCacheByAccountName(accountName)
	if u == nil {
