...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND (`created_at` <= ? AND `id` < ? AND `id` > ?) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
		...
		// 1st page
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND id > 48000 ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			ItemsPerPage+1,
		)
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND parent_category_id = ? AND (`created_at` <= ? AND `id` < ? AND `id` > ?) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			rootCategory.ID,
			time.Unix(createdAt, 0),
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	itemIDs := FetchItemIDsFromItems(items)
	userIDs := FetchUserIDsFromItems(items)
	userDict, err := FetchUserSimplesDictFromIDs(userIDs)
	if err != nil {
		logger.Info(err)
	...
	for _, item := range items {
		...
		if item.BuyerID != 0 {
			buyer, ok := userDict[item.SellerID]
			if !ok {
